(() => {
    ...
    const setupSlideCarousel = (root) => {
        ...
        root.querySelectorAll('.hero-actions a').forEach((action) => {
            action.addEventListener('pointerdown', stop);
            action.addEventListener('focus', stop);
            action.addEventListener('pointerenter', stop);
            action.addEventListener('pointerleave', start);
        });
    ...
    document.querySelectorAll('[data-shop-parts]').forEach((button) => {
        button.addEventListener('click', (event) => {
            const target = new URL(button.href, window.location.href);
            ...
            if (sameBasePage) {
                ...
                if (scrollToProducts()) {
                    window.history.replaceState(null, '', `${current.pathname}#products`);
                }
            }
            // If filters/query params are currently active, allow normal navigation
            // to the clean storefront URL so all products are restored.
        });
    });

    document.querySelectorAll('[data-browse-product]').forEach((button) => {
        button.addEventListener('click', () => {
            // Keep normal link navigation. The product_id query guarantees one exact item.
            // Stopping carousel interaction above prevents the slide changing mid-click.
        });
    });
    ...
    const exactProductId = new URL(window.location.href).searchParams.get('product_id');
    if (exactProductId) {
        const targetCard = document.querySelector(`#product-${CSS.escape(exactProductId)}`);
        if (targetCard) {
            targetCard.classList.add('target-product');
            window.setTimeout(() => {
