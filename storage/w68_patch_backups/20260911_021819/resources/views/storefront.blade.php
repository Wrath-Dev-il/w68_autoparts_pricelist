...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/w68-storefront.css') }}?v=20260911-green-v3">
</head>
<body>
        ...
        <section class="shell hero-grid">
            ...
            <div class="hero-side">
                <article class="promo-card promo-green">
                    ...
                    <strong>Real catalog data</strong>
                    <small>Products from core4_masterlist</small>
                </article>
        ...
        <section class="shell marketplace-section description-section" id="descriptions">
            <div class="section-heading compact-heading">
                <div>
                    ...
                    <p>All descriptions used by products in the current catalog.</p>
                </div>
            </div>
            <form action="{{ route('storefront') }}" method="GET" class="description-filter-form">
                @if ($search !== '')<input type="hidden" name="q" value="{{ $search }}">@endif
                @if ($brand !== '')<input type="hidden" name="brand" value="{{ $brand }}">@endif
                @if ($applicationSearch !== '')<input type="hidden" name="application" value="{{ $applicationSearch }}">@endif
                @if ($partNumberSearch !== '')<input type="hidden" name="part_number" value="{{ $partNumberSearch }}">@endif
                @if ($positionSearch !== '')<input type="hidden" name="position" value="{{ $positionSearch }}">@endif
                <select name="description" onchange="this.form.submit()">
                    <option value="">All Descriptions</option>
                    @foreach ($descriptions as $itemDescription)
                        <option value="{{ $itemDescription }}" @selected($description === $itemDescription)>{{ $itemDescription }}</option>
                    @endforeach
                </select>
                @if ($description !== '')
                    <a href="{{ route('storefront', array_filter(['q' => $search, 'brand' => $brand, 'application' => $applicationSearch, 'part_number' => $partNumberSearch, 'position' => $positionSearch])) }}">Clear description</a>
                @endif
            </form>
        </section>

        @if (count($newItems))
            <section class="shell marketplace-section newly-section" id="new-items">
                <div class="section-heading compact-heading">
                    <div>
                        <span class="section-kicker hot">NEW THIS YEAR</span>
                        <h2>Newly Added Items {{ date('Y') }}</h2>
                        <p>Shown one at a time and rotating every 2 seconds.</p>
                    </div>
                    <a class="view-all" href="#products">See all products →</a>
                </div>

                <div class="new-carousel" data-slide-carousel data-interval="2000">
                    <button class="carousel-arrow previous" type="button" data-slide-prev aria-label="Previous new item">‹</button>
                    <div class="new-slides" data-slide-track>
                        @foreach ($newItems as $index => $product)
                            <article class="new-slide {{ $index === 0 ? 'active' : '' }}" data-slide>
                                ...
                                <div class="new-slide-copy">
                                    <span>{{ $product->display_category }}</span>
                                    <h3>{{ $product->name }}</h3>
                                    <div class="new-slide-code">{{ $product->product_code }}</div>
                                    @if ($product->part_number)<p><b>Part No.</b> {{ $product->part_number }}</p>@endif
                                    @if ($product->application)<p><b>Application</b> {{ $product->application }}</p>@endif
                                    @if ($product->position)<p><b>Position</b> {{ $product->position }}</p>@endif
                                    <div class="new-slide-bottom">
                                        <strong>₱{{ number_format((float) $product->display_price, 2) }}</strong>
                                        <button type="button" class="add-cart" data-add-cart data-id="{{ $product->id }}" data-name="{{ $product->name }}" data-price="{{ $product->display_price }}">Add</button>
                                    </div>
                                </div>
                            </article>
                        @endforeach
                    </div>
                    <button class="carousel-arrow next" type="button" data-slide-next aria-label="Next new item">›</button>
                </div>
            </section>
        @endif
    ...
    <footer id="footer">
        <div class="shell footer-grid">
            ...
            <div>
                ...
                <a href="#new-items">New This Year</a>
            </div>
            <div>
                <strong>W68 Autoparts</strong>
    ...
    <script src="{{ asset('js/w68-storefront.js') }}?v=20260911-green-v3" defer></script>
</body>
</html>
