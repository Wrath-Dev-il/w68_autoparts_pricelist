(() => {
    ...
    const renderNavSuggestions = (items) => {
        ...
        navSuggestions.innerHTML = items.map((item) => {
            ...
            const visibleCode = /^SHOPEE-/i.test(rawCode) ? '' : rawCode;
            const code = visibleCode.replace(/[<>&"]/g, (char) => ({
                '<': '&lt;', '>': '&gt;', '&': '&amp;', '"': '&quot;'
            }[char]));
            ...
            return `
                <a class="search-suggestion-item" href="${url}" role="option">
                    <div class="search-suggestion-copy">
                        ...
                        <span>${[code, part ? `Part No. ${part}` : ''].filter(Boolean).join(' • ')}</span>
                    </div>
    ...
    const productModalCodeRow = productModal?.querySelector('[data-product-modal-code-row]');
    ...
    const openProductModal = (card) => {
        ...
        if (productModalDescription) productModalDescription.textContent = data.productDescription || '—';
        ...
        if (productModalCodeRow) productModalCodeRow.hidden = !data.productCode;
