...
class CustomerHomeController extends Controller
{
    public function index(Request $request): View|RedirectResponse
    {
        ...
        try {
            ...
            $filterOptions = [
                'brands' => $this->distinctValues('p.category'),
                'descriptions' => $this->distinctValues('p.description'),
                'positions' => $this->distinctValues('p.Position'),
                'applications' => $this->distinctValues('p.application'),
            ];
    ...
    public function searchSuggestions(Request $request): JsonResponse
    {
        ...
        $rows = DB::connection('masterlist')
            ...
            ->where(function ($query) use ($contains) {
                $query->where('p.product_code', 'like', $contains)
                    ...
                    ->orWhere('p.specification', 'like', $contains);
            })
            ->select([
                'p.id',
                'p.product_code',
            ...
            ->orderByRaw(
                'CASE '
                . 'WHEN p.product_code LIKE ? THEN 0 '
    ...
    private function applyFilters(Builder $query, array $filters): void
    {
        if ($filters['q'] !== '') {
            ...
            $query->where(function ($inner) use ($term) {
                $inner->where('p.product_code', 'like', $term)
                    ->orWhere('p.part_number', 'like', $term)
                    ->orWhere('p.description', 'like', $term)
                    ->orWhere('p.category', 'like', $term)
                    ->orWhere('p.application', 'like', $term)
                    ->orWhere('p.Position', 'like', $term)
                    ->orWhere('p.specification', 'like', $term);
            });
        }
