(() => {
    ...
    /*
     * Explicit folded-summary listener fixes desktop/laptop clicks.
     * Do not depend only on event bubbling from the <header>.
     */
    collapsedNavSummary?.addEventListener('click', (event) => {
        expandFoldedNavigation(event);
    });

    collapsedNavSummary?.addEventListener('keydown', (event) => {
    ...
    /*
     * Keep a header-level fallback so blank folded-nav space is also
     * clickable on desktop and touch devices.
     */
    nav?.addEventListener('click', (event) => {
        if (event.target.closest('[data-nav-expand-trigger]')) {
            return;
        }
