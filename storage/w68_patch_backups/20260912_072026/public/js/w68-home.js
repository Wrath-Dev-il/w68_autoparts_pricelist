(() => {
    ...
    const hideLoader = () => {
        ...
        if (loaderRestartTimer) {
            window.clearTimeout(loaderRestartTimer);
        }
    ...
    const showLoader = () => {
        ...
        /*
         * Safety net for Safari/iPad:
         * if navigation is cancelled, converted into same-document scrolling,
         * or restored from BFCache, never leave the W68 animation stuck.
         */
    ...
    /*
     * Safari/iPad can restore this page from BFCache without another normal
     * load event. pageshow fires for both normal and BFCache restoration.
     */
    window.addEventListener('pageshow', () => {
        hideLoader();

        /*
         * Lazy images can be paused while Safari restores a cached page.
         * Re-assign a missing/broken image source once the page is visible.
         */
        document.querySelectorAll('img[data-w68-product-image]').forEach((image) => {
            if (image.complete && image.naturalWidth > 0) {
                return;
            }

            const source = image.dataset.w68ProductImage || image.src;
    ...
    document.querySelectorAll('[data-loading-form]').forEach((form) => {
        form.addEventListener('submit', (event) => {
            /*
             * For GET forms, calculate the destination first.
             * If it resolves to this same document, don't trap the user
             * behind a loader just for an anchor/location update.
             */
            if (
    ...
    document.querySelectorAll('[data-loading-link]').forEach((link) => {
        link.addEventListener('click', (event) => {
            ...
            /*
             * NEW ITEMS on an already-unfiltered page is only a #new-items
             * move. CLEAR on an already-clear page is only #products.
             * Never show a full-page loader for those same-document actions.
             */
            if (sameDocumentTarget(target)) {
                hideLoader();
    ...
    /*
     * Mark every Home product thumbnail with its source so Safari pageshow
     * can recover it if the browser restored a blank lazy image.
     */
    document.querySelectorAll(
        '.new-item-card img, .product-card .product-image-wrap img'
    ).forEach((image) => {
        image.dataset.w68ProductImage = image.currentSrc || image.src;
    });
    ...
    const expandNav = (manual = false) => {
        if (!nav) return;

        if (manual) {
            /*
             * Desktop Chrome may change scrollY when this sticky navbar
             * becomes taller. Without a lock, that synthetic/layout-shift
             * scroll can immediately trigger collapseNav() again.
             */
            manualNavExpandUntil = Date.now() + 1200;
        }
        ...
        /*
         * Reset the scroll baseline AFTER expansion so the navbar height
         * change is not mistaken for user downward scrolling.
         */
        window.requestAnimationFrame(() => {
    ...
    /*
     * DESKTOP CHROME HARD FALLBACK
     *
     * Detect mouse-down by coordinates instead of depending only on the
     * event target. This still works if a CSS layer inside the folded header
     * becomes the hit target.
     *
     * The cart rectangle is excluded so cart clicks continue to open cart.
     */
    document.addEventListener(
        ...
        (event) => {
            ...
            const foldedCart = nav.querySelector(
                '.nav-cart-button-collapsed[data-cart-open]'
            );

            if (foldedCart) {
                const cartRect = foldedCart.getBoundingClientRect();
