(() => {
    ...
    const collapsedNavOpen = document.querySelector('[data-nav-collapsed-open]');
    ...
    const openCollapsedNavigation = (event = null) => {
        ...
        window.setTimeout(() => {
            const firstSearch = nav.querySelector('[data-recommend-input]');
    ...
    const updateQuantityModalTotal = (normalize = false) => {
            ...
            if (normalize) {
                ...
                if (total) {
                    total.textContent = money(
                        Number(pendingQuantityProduct.price || 0)
                    );
                }
            } else if (total) {
        ...
        if (total) {
            total.textContent = money(
                Number(pendingQuantityProduct.price || 0) * qty
            );
        }
    };
